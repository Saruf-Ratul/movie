				...
				private function pagination($con,$table,$pno,$n){
					...
					if ($last != 1) {
						if($pageno >1) {
							$previous = "";
							$previous = $pageno - 1;
							$pagination .= "<li class='page-item'><a class='page-link' pn='".$previous."' href='#' style='color:#333;'><button>Previous</button><a/></li></li>";
						}
      	...
      	for ($i=0; $i < count($ar_price); $i++){
           ...
           $rem_qty = $ar_qty[$i];
            if ($rem_qty < 0){
      		...
      		$insert_product = $this->con->prepare("INSERT INTO `invoice_details`(`invoice_no`, `product_name`, `price`, `qty`, `coustomer_name`, `vendor_paid`, `fly_d`, `country`, `air`, `hotel`, `sanction_dat`, order_date) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
            $insert_product->bind_param("isddsdssssss", $invoice_no,$ar_pro_name[$i],$ar_price[$i],$ar_qty[$i],$ar_coustomer_name[$i],$ar_vendor_paid[$i],$ar_fly_d[$i],$ar_country[$i],$ar_air[$i],$ar_hotel[$i],$ar_sanction_dat[$i],$order_date);
